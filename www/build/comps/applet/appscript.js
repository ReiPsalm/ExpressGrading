            ...
            $.ajax({
                ...
                success: function(data){
                    if(data == "1"){
                        ...
                        Appex.Notifier('info','Error Login','build/library','top right','User does not exist!');
                        $('#Expform').trigger('reset');
                    }else if(data == "0"){
                        Appex.Notifier('warning','Data collpase','build/library','top right','Please restart the application')
                    }else{
                        Appex.Notifier('error','Action unkown','build/library','top right','Please try again!')
                    }
                }
            });
        });
